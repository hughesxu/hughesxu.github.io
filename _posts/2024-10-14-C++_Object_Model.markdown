...
C++对象模型是个常见、且复杂的话题，本文基于[Itanium C++ ABI](https://itanium-cxx-abi.github.io/cxx-abi/abi.html)通过代码实践介绍了几种C++继承场景下对象模型，尤其是存在虚函数继承的情况下，并通过图的方式直观表示内存的布局。  
...
```
gcc编译器可通过-fdump-class-hierarchy参数，查看类的内存布局。可以得到如下信息：
```
...
```
从类`Base_C`的定义来看，类占用的空间包括一个虚函数表指针vptr和一个整型变量。由于内存对齐的原因，类占用16字节。  
...
```
bool show_vtable_entry(VTABLE_PTR start, size_t idx)
{
    void *func = start[idx];
    Dl_info func_info;
